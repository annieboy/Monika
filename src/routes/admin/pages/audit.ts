...
export async function auditLogPage(prisma: PrismaClient, query: Record<string, string>): Promise<string> {
  ...
  const body = `
    <form class="search-bar" method="GET" action="/admin/audit" style="align-items:center">
      <select name="eventType" style="padding:.5rem .875rem;border:1px solid #e2e8f0;border-radius:.5rem;font-size:.875rem">
        <option value="">All event types</option>
        ${typeOptions}
      </select>
      <button type="submit">Filter</button>
      ${eventType ? `<a href="/admin/audit" style="padding:.5rem .875rem;color:#64748b;font-size:.875rem">Clear</a>` : ''}
    </form>
    ...
    <div class="card">
      ...
      ${paginate(total, page, PAGE_SIZE, eventType ? `/admin/audit?eventType=${encodeURIComponent(eventType)}` : '/admin/audit')}
    </div>
  `
